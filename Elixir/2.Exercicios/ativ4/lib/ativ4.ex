defmodule Ativ4 do
  ...
  """
  def map([], _f), do: []
  def map([x | resto], f), do: [f.(x) | map(resto, f)]
  ...
  def fold_right([x | resto], ini, f), do: f.(x, fold_right(resto, ini, f))
  ...
  def fold_left([x | resto], ini, f), do: fold_left(resto, f.(ini, x), f)
  ...
  """
  def concat_strings_sep([x | resto], sep), do: fold_left(resto, x, fn(x,y) -> x <> sep <> y end)
  def concat_strings_sep(ls, sep), do: fold_left(ls, "", fn(x,y) -> x <> sep <> y end)
  ...
  [1, 3, 5]
  """
  def par_3n_mais_1(l), do: filter(l, fn(x) -> rem(3*x+1, 2) == 0 end)
end
